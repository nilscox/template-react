...
describe('ReplayCommit', () => {
  const apply = (...steps: ReplayStepData[]) => {
    const editor = new MemoryEditor();
    const commit = ReplayCommit.create({ name: 'commit', steps });

    commit.apply(editor);

    return commit.data.steps;
  };
  ...
  it('applies one action', () => {
    ...
    const playedSteps = apply({ name: 'step', actions: [action1] });
  ...
  it('applies multiple actions in a single step', () => {
    const playedSteps = apply({ name: 'step', actions });
